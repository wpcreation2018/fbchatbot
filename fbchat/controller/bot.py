...
remote_img_urls = {
    ...
    'img_transfer'  : 'https://i.ibb.co/rtKXPrD/transfer.png',
...
local_img_urls = {
    ...
    'img_transfer'  : 'imgs/transfer.png',
...
class EchoBot(Client):
    def onMessage(self, author_id, message_object, thread_id, thread_type, **kwargs):
        ...
        if message_object.text in askcod_list:
            ...
            self.sendRemoteFiles(remote_img_urls['img_cod'], thread_id=thread_id, thread_type=thread_type)
        ...
        if message_object.text in askmore_list:
            ...
            self.send(Message(text=tpl_urls['shop_page']), thread_id=thread_id, thread_type=thread_type)
            #self.sendLocalImage(local_img_urls['imgs/discount.jpg'], thread_id=thread_id, thread_type=thread_type)
            self.sendRemoteFiles(remote_img_urls['img_discount'], thread_id=thread_id, thread_type=thread_type)            
        ...
        if message_object.text in askprice_list:
            ...
            self.send(Message(sticker=Sticker("2379835878707816")), thread_id=thread_id, thread_type=thread_type)
        ...
        if message_object.text in asksizebig_list:
            ...
            self.send(Message(text=ResponseBy.goto_website()), thread_id=thread_id, thread_type=thread_type)
            self.send(Message(text=tpl_urls['bigsize_page']), thread_id=thread_id, thread_type=thread_type)
        ...
        if message_object.text in cod_list:
            ...
            self.sendRemoteFiles(remote_img_urls['img_cod'], thread_id=thread_id, thread_type=thread_type)
            self.sendRemoteFiles(remote_img_urls['img_claim'], thread_id=thread_id, thread_type=thread_type)            
        ...
        if message_object.text in thankful_list:
            ...
            self.send(Message(text=ResponseBy.goto_website()), thread_id=thread_id, thread_type=thread_type)
            self.send(Message(text=tpl_urls['home_page']), thread_id=thread_id, thread_type=thread_type)
            #self.sendLocalImage(local_img_urls['imgs/discount.jpg'], thread_id=thread_id, thread_type=thread_type)
            self.sendRemoteFiles(remote_img_urls['img_discount'], thread_id=thread_id, thread_type=thread_type)
            self.markAsRead(thread_id)
        ...
        if message_object.text in transfer_list:
            ...
            #self.sendLocalImage(local_img_urls['imgs/claim.png'], thread_id=thread_id, thread_type=thread_type)
            self.sendRemoteFiles(remote_img_urls['img_transfer'], thread_id=thread_id, thread_type=thread_type)
            self.sendRemoteFiles(remote_img_urls['img_claim'], thread_id=thread_id, thread_type=thread_type)
        ...
        if message_object.text == 'tq':
            ...
            self.sendRemoteFiles(remote_img_urls['img_thxq'], thread_id=thread_id, thread_type=thread_type)
            self.send(Message(text="📣 ช่วงนี้ เฟสบุ๊คทำการบล๊อคบัญชีบ่อย น้องขอความร่วมมือคุณพี่ แอดไลน์ 👉🏻📱 Line Official @theperfectladies\n\nหรือ กดลิ้งค์ด้านล่างนี้จ้า\n⬇⬇⬇\n\nhttp://nav.cx/8rtmakW\n\nได้ไหมจ้า ? เพื่อป้องกันไม่ให้พลาดการติดต่อจ้า 🙏🏻"), thread_id=thread_id, thread_type=thread_type)
            self.markAsRead(thread_id)
        ...
        if message_object.text in sku_twinsshop:
            ...
            self.sendRemoteFiles(tw.response_image(), thread_id=thread_id, thread_type=thread_type)
